    ...
    IPayoutHandler
{
    ...
    internal static decimal GetBaseBlockReward(GethChainType chainType, ulong height)
    {
        switch(chainType)
        {
			...
			case GethChainType.Dubx:
               return DubxConstants.BaseRewardInitial;
			   
			case GethChainType.Powblocks:
			if(height >= PowblocksConstants.oneHeight)
					return PowblocksConstants.oneHeightReward;
			if(height >= PowblocksConstants.twoHeight)
					return PowblocksConstants.twoHeightReward;
			if(height >= PowblocksConstants.threeHeight)
					return PowblocksConstants.threeHeightReward;
			if(height >= PowblocksConstants.fourHeight)
					return PowblocksConstants.fourHeightReward;
			if(height >= PowblocksConstants.fiveHeight)
					return PowblocksConstants.fiveHeightReward;
